...
def fetch_weibo_posts(keyword, start_date, end_date, pages):
    """从微博网页抓取带有关键词和日期范围的微博数据"""
    weibo_posts = []
    for page in range(1, pages+1):
        url = f"https://s.weibo.com/weibo?q={keyword}&typeall=1&suball=1&timescope=custom:{start_date}:{end_date}&page={page}&Refer=g"
...
def main():
    ...
    for keyword in keywords:
        posts = fetch_weibo_posts(keyword, start_date, end_date, pages)
        if not posts:
